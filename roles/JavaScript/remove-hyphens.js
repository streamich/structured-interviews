...
const remove = (obj) => {
  ...
  else if (obj instanceof RegExp) return new RegExp(remove(obj.source));
  else if (obj instanceof Array) return obj.map(remove);
  else if (obj instanceof Object) return Object.keys(obj).reduce((acc, k) => ({...acc, [remove(k)]: remove(obj[k])}), {});
